...
class LinearModel(object):
    def __init__(self, input_dim, batch_size, loss_type, update_rule, learning_rate=0.001, regularization=None, regularizing_constant=0.01):
    ...
    def calculate_loss(self, update_parameter=True):
        ...
        loss_value = self.loss(labels =self.labels, targets=self.targets, parameters=self.parameters, regularizing_constant=self.regularizing_constant)
    ...
    def update_with_gradients(self):
        ...
        if self.update_rule == "Matrix" or self.update_rule == "Closed form":
            ...
            if self.regularization == "L2":
                ...
                self.parameters = (self.inputs.t().mm(self.inputs) + self.regularizing_constant*self.regularizing_constant*torch.eye(self.input_dim+1)).inverse().mm(self.inputs.t()).mm(self.labels.reshape([-1,1]))
        if self.update_rule == "GD" or self.update_rule == "SGD":
            if not self.regularization:
                gradients = -2*self.inputs.t().mm((self.labels - self.targets).reshape([-1,1])) #TODO: should it be divided with batch size
            ...
            if self.regularization == "L1":
                gradients = -2*self.inputs.t().mm((self.labels - self.targets).reshape([-1,1])) + \
                (self.regularizing_constant/self.batch_size)*torch.sign(self.parameters) # USing naive update rule
